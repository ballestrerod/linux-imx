...
/* POE LEDs triggers */
static int poe_led_trigger_activate(struct led_classdev *led_cdev)
{
...
static void tps2388_poll_function(struct timer_list *t)
{
	struct tps2388 *data = from_timer(data, t, poll_timer);
...
#else
	timer_setup(&tps->poll_timer, tps2388_poll_function, (unsigned long)tps);
#endif
